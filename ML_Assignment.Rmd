...
pokemon_data_clean = pokemon_data %>% 
  ...
  dplyr::select(-c(classification, is_legendary)) # TODO: Decide to keep classification or not. Currently removed due to many small classes 
...
pokemon_data_pairs = pokemon_data_clean %>% 
  dplyr::select(-c(type1, generation, percentage_male_cat))
...
tuneGrid = expand.grid(mtry = c(1:ncol(pokemon_data_clean)))
