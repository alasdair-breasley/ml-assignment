...
# Summarise numeric variables and export to csv 
numeric_summary_data = summary(pokemon_data_clean_numeric)

numeric_summary_df = data.frame(matrix(ncol = length(numeric_summary_data) / 6, nrow = 6))

for (i in 1:(length(numeric_summary_data) / 6)) {
  
  numeric_summary_df[, i] = numeric_summary_data[(1 + (6 * (i - 1))):((1 + (6 * (i - 1))) + 5)]
  
}

colnames(numeric_summary_df) = names(pokemon_data_clean_numeric)

numeric_summary_df_clean = as.data.frame(lapply(numeric_summary_df, function(x) gsub(".*:", "", x)))

rownames(numeric_summary_df_clean) <- c("Min.", "1st Qu.", "Median", "Mean", "3rd Qu.", "Max.")

## Write the summary data frame to a CSV file
#write.csv(df_modified, "summary_output.csv", row.names = TRUE)
...
for (ntrees in c(500, 1000, 2000, 4000)) {
  ...
  print(round(confusionMatrix(predictions, test_data$type1)$overall[[1]], 3))
  
  if (ntrees == 1000) { 
    
    cm = confusionMatrix(predictions, test_data$type1)$table
...
# cm = confusionMatrix(predictions, test_data$type1)$table
# 
# df_1 = as.data.frame(cm)
# 
# write.csv(df_1, "cm_dt4.csv", row.names = TRUE)
