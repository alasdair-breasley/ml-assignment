...
pokemon_data_clean = pokemon_data %>% 
  ...
  dplyr::select(-classification) # TODO: Decide to keep classification or not. Currently removed due to many small classes 
...
pokemon_data_pairs = pokemon_data_clean %>% 
  dplyr::select(-c(type1, generation, is_legendary, percentage_male_cat))
...
# Sample indices based on the test split proportion 
...
for (ntrees in c(500, 1000, 2000, 4000)) {
  ...
  model_rf_nt = randomForest(type1 ~ ., data = training_data, ntree = ntrees, mtry = model_rf$bestTune[[1]])
